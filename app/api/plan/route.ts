...
export async function GET(){try{await ready();const plan=await env.DB.prepare("SELECT * FROM training_plans WHERE plan_key = ?").bind("primary").first();return Response.json({plan:plan||{name:"4-day strength split",days:JSON.stringify(defaultDays)}})}catch(error){return Response.json({error:error instanceof Error?error.message:"Could not load plan"},{status:500})}}
export async function PUT(request:Request){try{await ready();const body=await request.json() as {name?:string;days?:unknown};if(!Array.isArray(body.days)||!body.days.length)return Response.json({error:"Add at least one workout day"},{status:400});await env.DB.prepare(`INSERT INTO training_plans (plan_key,name,days,updated_at) VALUES (?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(plan_key) DO UPDATE SET name=excluded.name,days=excluded.days,updated_at=CURRENT_TIMESTAMP`).bind("primary",body.name?.trim()||"My training split",JSON.stringify(body.days)).run();return Response.json({saved:true})}catch(error){return Response.json({error:error instanceof Error?error.message:"Could not save plan"},{status:500})}}
