...
export async function PUT(request: Request) {
  try {
    await ready();
    const body = await request.json() as { profileId?:number; exercises: unknown; completedSets: number; totalSets: number; volume: number; };
    ...
    await env.DB.prepare(`INSERT INTO workouts (workout_key, name, muscle_group, exercises, status, completed_sets, total_sets, volume, workout_date, updated_at, profile_id)
      ...
      .bind(currentKey, "Lower body strength", "Legs", JSON.stringify(body.exercises), "active", body.completedSets || 0, body.totalSets || 0, body.volume || 0, date,profileId).run();
...
export async function POST(request: Request) {
  try {
    ...
    await env.DB.prepare("INSERT INTO workouts (workout_key, name, muscle_group, exercises, status, completed_sets, total_sets, volume, workout_date, profile_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
      .bind(`finished-${now.getTime()}`, "Lower body strength", "Legs", JSON.stringify(body.exercises), "completed", body.completedSets || 0, body.totalSets || 0, body.volume || 0, now.toISOString().slice(0,10),body.profileId||1).run();
